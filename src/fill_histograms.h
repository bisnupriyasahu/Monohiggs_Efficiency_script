void mutau_analyzer::fillHist( string histNumber , int muIndex, int tauIndex, bool isFakeBkg, float event_weight){
  ...
  float relMuIso_new = ( muPFChIso->at(muIndex) + max( muPFNeuIso->at(muIndex) + muPFPhoIso->at(muIndex) - 0.5 *muPFPUIso->at(muIndex) , 0.0 ) - my_tauP4.Pt() ) / (my_muP4.Pt());
  ...
  float metBins[11]={ 105, 120, 150, 180, 210, 235, 260, 285, 325, 400, 2000};
  plotFill_customBinning("metFull_"+hNumber, met_with_overflow , 10, metBins,  event_weight);
    
  double mumet_trmass = mT_muMet;
  if(mumet_trmass >= 2000)
    mumet_trmass = 1900;
  plotFill_customBinning("muMetTMassFull_"+hNumber, mumet_trmass , 12, TrMassBins,  event_weight);
  ...
  // //if(debug)cout <<"plots filled for "<<hNumber<<endl;
    plotFill("muPt_new_"+hNumber,  my_muP4.Pt() , 98 , 20.0 , 1000,  event_weight);
  plotFill("muMass_new_"+hNumber,  my_muP4.M() , 10 , 0.0 , 1,  event_weight);
  plotFill("tauPt_new_"+hNumber,  my_tauP4.Pt() , 97 , 30 , 1000,  event_weight);
  plotFill("tauMass_new_"+hNumber,  my_tauP4.M() , 20 , 0.0 , 2.0,  event_weight);
  plotFill("met_new_"+hNumber, my_metP4.Pt() , 90, 100, 1000,  event_weight);
  plotFill("visMass_new_"+hNumber, visMass_mutau , 95, 50, 1000,  event_weight);
  plotFill("higgsPt_new_"+hNumber,HiggsPt ,  100, 0, 1000,  event_weight);

  plotFill_2D("visMass_met_new_"+hNumber, visMass_mutau, my_metP4.Pt() , 95, 50, 1000, 95, 50, 1000, event_weight);
  plotFill_2D("higgsPt_met_new_"+hNumber, HiggsPt, my_metP4.Pt() , 95, 50, 1000, 95, 50, 1000, event_weight);

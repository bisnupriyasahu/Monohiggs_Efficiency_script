void mutau_analyzer::selections(float weight, int shift, string uncObject)
{
  ...
  for(int i=0; i<nMC; i++){
    ...
    if(gentau1_index >= 0 and gentau2_index >= 0)
    {
      ...
      double genTauPt = gentau1.Pt();
      double gensubleadingtauPt = gentau2.Pt();
	      ...
	      if(gentau1.Pt()>20 && gentau2.Pt()>20)
		    {
		      ...
		      if (nMu>0 && nTau > 0)
		      {
		        ...
		        if (mu_index >= 0 && tau_index >= 0)
		        {
		          ...
		          float TMass = TotTMass_F(my_muP4,my_tauP4,my_metP4);
              if(TMass < 60)
              {                
               //genmatching loop:
		          ...
		          if(genmatchedmu_idx >= 0 && genmatchedtau_idx >= 0)
			        {
			          ...
			          plotFill("gentauPt_num_1", gen_subleadingtauPt, 970, 30, 1000, event_weight);
                ...
                double mvis = ( genmatch_mup4 + genmatch_taup4).M();
			          double higgsPt = (genmatch_mup4 + genmatch_taup4).Pt();
			          double tot_tr_mass = (genmatch_mup4 + genmatch_taup4 + my_metP4).M();
			          float deltaR = genmatch_mup4.DeltaR(genmatch_taup4);

                plot_resolved_taus(genmatch_mup4, genmatch_taup4, tau_index, "4", event_weight);
                if (thirdLeptonVeto(mu_index,tau_index ))
		          	{
                  plot_resolved_taus(genmatch_mup4, genmatch_taup4, tau_index, "5", event_weight);
                  bool pass_bjet_veto = ((bJet_medium(mu_index, tau_index).size() == 0) && (bJet_loose(mu_index, tau_index).size() < 2));
				          if (pass_bjet_veto)
				          {
                    plot_resolved_taus(genmatch_mup4, genmatch_taup4, tau_index, "6", event_weight);
					          if (higgsPt > 65)
					          {
                      plot_resolved_taus(genmatch_mup4, genmatch_taup4, tau_index, "7", event_weight);
                      if (mvis < 125)
						          {
                        plot_resolved_taus(genmatch_mup4, genmatch_taup4, tau_index, "8", event_weight);
                        if (my_metP4.Pt() > 105)
							          {
                          plot_resolved_taus(genmatch_mup4, genmatch_taup4, tau_index, "9", event_weight);
                        }
                      }
                    }
                  }
    ...
    if( abs(gentau1.Eta())<2.3 && abs(gentau2.Eta())<2.3 && gentau1.Pt()>20 && gentau2.Pt()>20)
	  {
	    cout<<__LINE__<<endl; // 
	    plotFill("genmuPt_boostedraw_1", genTauPt, 970, 30, 1000, event_weight);
	    plotFill("gentauPt_boostedraw_1", gensubleadingtauPt, 970, 30, 1000, event_weight);
	    ...
	    if (nMu> 0 && nBoostedTau > 0 )
	    {
          ...
          float TMass = TotTMass_F(myboost_muP4,myboost_tauP4,myboost_metP4);
          if(TMass < 60)
          {
	          //gen matching loop
	          cout<<__LINE__<<endl; // 	
          ...
          if (genmatchedmu_idx >= 0 && genmatchedtau_idx >=0)
            ...
            //myboost_metP4 = SetPtEtaPhiE(pfMET, 0, pfMETPhi, pfMET);
	          ...
	          plotFill("genmuPt_boostedraw_2", gen2_TauPt, 970, 30, 1000, event_weight);
	          plotFill("gentauPt_boostedraw_2", gen2_subleadingtauPt, 970, 30, 1000, event_weight);
            ...
            double mvis = ( genmatchboost_mup4 + genmatchboost_taup4).M();
			      double higgsPt = (genmatchboost_mup4 + genmatchboost_taup4).Pt();
			      double tot_tr_mass = (genmatchboost_mup4 + genmatchboost_taup4 + myboost_metP4).M();
			      float deltaR = genmatchboost_mup4.DeltaR(genmatchboost_taup4);
            plot_boosted_taus(genmatchboost_mup4, genmatchboost_taup4, tau_index , "4", event_weight);
            if (thirdLeptonVeto_boosted(mu_index, tau_index))
			      {
              plot_boosted_taus(genmatchboost_mup4, genmatchboost_taup4, tau_index , "5", event_weight);
              bool pass_bjet_veto = ((bJet_medium_boosted(mu_index, tau_index ).size() == 0) && (bJet_loose_boosted(mu_index,tau_index).size() < 2));
				      if (pass_bjet_veto)
              {
                plot_boosted_taus(genmatchboost_mup4, genmatchboost_taup4, tau_index , "6", event_weight);
                if (higgsPt > 65)
					      {
                  plot_boosted_taus(genmatchboost_mup4, genmatchboost_taup4, tau_index , "7", event_weight);
                  if (mvis < 125)
					      	{
                    plot_boosted_taus(genmatchboost_mup4, genmatchboost_taup4, tau_index , "8", event_weight);
                    if (myboost_metP4.Pt() > 105)
                    {
                      plot_boosted_taus(genmatchboost_mup4, genmatchboost_taup4, tau_index , "9", event_weight);
                    }
                  }
                }
              }
...
cout<<__LINE__<<endl; //
  ...
  std::cout<<"n boosted is "<<nBoostedTau<<"\n" << "nboostedorg is "<<nBoostedTauOrig<<"\n" <<"ntau is "<<nTau<<std::endl;
  if (nMu > 0 && nBoostedTau > 0)
  {
    for (int i = 0; i < nMu; i++)
	  {

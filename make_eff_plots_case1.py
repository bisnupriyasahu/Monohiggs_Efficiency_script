...
for idx in mzp_map:
	...
	for var in vars:
		...
		#hist_2 = infile.Get('gen'+var+'_boostedraw_1')
		
                #tmpHist_num1.Divide(tmpHist_den)
                #tmpHist_num2.Divide(tmpHist_den)

		if var != 'deltaR':
                        #tmpHist_num1 = hist_1.Rebin(22, 'gen'+var+'_num_1')
			tmpHist_num1 = hist_1
			#tmpHist_num2 = hist_1.Rebin(22, 'gen'+var+'_boostedraw_1', new_binning)
                        tmpHist_den = hist_raw  
		'''else:
			dr_bins = 5
			tmpHist_num1 = hist_1.Rebin(dr_bins, 'gen'+var+'_num_1')
			#tmpHist_num2 = hist_1.Rebin(dr_bins, 'gen'+var+'_boostedraw_1')
		...
		# 	var = 'tauPt'
		tmpHist_num1.SetTitle(var+ ' efficiency case1 tau isolation '+signal)
		...
		legende=make_legend()
		legende.AddEntry(tmpHist_num1,"raw eficiency","lp")
		...
		canv.SaveAs('plots_case1/eff_2017mutau_'+signal+'_'+var+'_'+'.png')
